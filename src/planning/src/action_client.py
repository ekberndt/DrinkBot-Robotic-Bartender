...
def main(start_pos, end_pos):
    ...
    goal.request.start_state.joint_state.position = [start_pos[0], start_pos[1], start_pos[2], start_pos[3], 0.0, 0.0, 0.0]
    ...
    target_joint_angles = [end_pos[0], end_pos[1], end_pos[2], end_pos[3], 0.0, 0.0, 0.0]
    tolerance = 0.0001
    consts = []
...
if __name__ == '__main__':
    str_arr = input("Give me start array: \n").split()
    start_pos = [float(x) for x in str_arr]
    end_pos = [float(x) for x in input("Give me end array: \n").split()]
    main(start_pos, end_pos)
